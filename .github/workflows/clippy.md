...
safe-outputs:
  messages:
    footer: "> Generated by Clippy at [{run-started}]({run_url}). [Learn more](https://github.com/githubnext/agentics/blob/main/docs/repo-assist.md)."
    run-started: "Clippy is processing {event_type}, started at at [{run-started}]({run_url})..."
    run-success: "✓ Clippy completed successfully at {run-ended}."
    run-failure: "✗ Clippy encountered {status} at {run-ended}."
  add-comment:
    max: 10
    target: "*"
