...
/// <summary>
/// Translates ISO/IEC 29500 Strict namespace URIs and relationship types to their
/// OOXML Transitional equivalents.
/// </summary>
/// <remarks>
/// The lookup tables are derived from the Open XML SDK's internal
/// <c>OpenXmlNamespaceResolver</c> (<c>_strictTransitionalNamespaces</c> and
/// <c>_strictTransitionalRelationshipPairs</c>) and represent the authoritative,
/// complete mapping defined by the spec.  A simple string-pattern replacement is
/// intentionally avoided because several entries deviate from the obvious pattern
/// (e.g. hyphens, different base URLs, and one spec-bug workaround).
/// </remarks>
internal static class StrictOoxmlTranslator
{
    ...
    private static readonly Dictionary<string, string> s_namespaces = new(StringComparer.Ordinal)
    {
        ...
        // Note: customProperties maps to "custom-properties" (hyphen), not "customProperties"
        ...
        // Note: extendedProperties maps to "extended-properties" (hyphen)
    ...
    private static readonly Dictionary<string, string> s_relTypes = new(StringComparer.Ordinal)
    {
        ...
        // Note: customProperties maps to "custom-properties" (hyphen)
        ...
        // Note: thumbnail maps to package/2006/..., not officeDocument/2006/...
    ...
    /// or the original URI unchanged if it is already Transitional.
    /// </summary>
    public static string TranslateNamespace(string namespaceUri) =>
        s_namespaces.TryGetValue(namespaceUri, out var transitional) ? transitional : namespaceUri;

    /// <summary>
    /// Returns the Transitional relationship type URI for a given Strict relationship type URI,
    /// or the original URI unchanged if it is already Transitional.
    /// </summary>
